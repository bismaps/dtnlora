...
try:
    ...
    while True:
        bpa.update()
        if is_timestamp_older_than_timeout(last_transmission, 5000):
            bundle_counter += 1
            simulated_water_level = 150 + (bundle_counter % 50)
            payload_data = message_str.format(simulated_water_level).encode('utf-8')
